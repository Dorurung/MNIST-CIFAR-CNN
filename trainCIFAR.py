...
optimizer = torch.optim.SGD(alexnet.parameters(), lr=learning_rate, momentum = 0.9, weight_decay=0.0005)
accuracy_over_epoch = {'train': [], 'val': []}

f = open(f"cifar dropout{dropout_rate} result.txt", 'w')
...
accuracy_over_epoch['train'] = [float(sum(correct))/sum(total) for correct, total in accuracy_over_epoch['train']]
accuracy_over_epoch['val'] = [float(sum(correct))/sum(total) for correct, total in accuracy_over_epoch['val']]
plt.plot(range(num_epochs), accuracy_over_epoch['train'])
plt.plot(range(num_epochs), accuracy_over_epoch['val'])
plt.savefig(f'learning_curve_alexnet{dropout_rate}.png')
torch.save(alexnet.state_dict(), f"model/cifar_{dropout_rate}.pt")
plt.show()